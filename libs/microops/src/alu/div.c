...
static inline int op(float a, float b) { return a + b; }
...
static void offset_and_write_reg(unsigned char *offset_addr1,
  ...
  if (ptr->is_immediate == true) {
    int res = op(*(float *)dest_reg->value, immediate_val);
    ...
    if (deref1) {
      ...
      int res = op(*(float *)offset_addr2, addr_as_int);
    ...
    int res = op(*(float *)offset_addr2, *(float *)offset_addr1);
    memcpy(offset_addr2, &res, ptr->size);
  }
}
static void offset_and_write_addr(unsigned char *offset_addr1,
  ...
  int res = op(*(float *)offset_addr1, *(float *)offset_addr2);
  memcpy(offset_addr2, &res, ptr->size);
}
static void offset_and_write_reg_from_reg(Register *dest_reg, Register *src_reg,
  ...
  if (deref2) { // has dest been dereferenced
    int calc = *(float *)(memory.data + *(int *)dest_reg->value + offset2);
    int res = op(calc, (float)*src_reg->value);
    memcpy(memory.data + *(int *)dest_reg->value + offset2, &res, 4);
  } else {
    int res = op((float)*dest_reg->value, (float)*src_reg->value);
    memcpy(dest_reg->value, &res, 4);
  }
}
